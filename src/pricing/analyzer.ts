...
import type { PricingConfig, PricingAnalysis, TierAnalysis, VariableCost, Tier } from './types';
...
function analyzeTier(
  ...
  const targetMargin = tier.targetMargin ?? (tier.price > 0 ? DEFAULT_TARGET_MARGIN : null);
  ...
  const maxVariableCost =
    tier.price * (1 - (targetMargin ?? DEFAULT_TARGET_MARGIN)) - fixedCostPerUser;
  const maxUnitsPerCost: Record<string, number> = {};
  const recommendedLimits: Record<string, number> = {};

  if (maxVariableCost <= 0) {
    warnings.push(`Fixed costs ($${fixedCostPerUser}) exceed margin budget. Tier not viable.`);
    return {
      name: tier.name,
      price: tier.price,
      targetMargin,
  ...
  const projectedMargin = tier.price > 0 ? (tier.price - projectedCost) / tier.price : null;
  const isViable = projectedMargin === null || projectedMargin >= MINIMUM_VIABLE_MARGIN;
...
export function formatAnalysis(analysis: PricingAnalysis): string {
  ...
  lines.push(analysis.allTiersViable ? '✅ All tiers viable' : '❌ Issues detected');
