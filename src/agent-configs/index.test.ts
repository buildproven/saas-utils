...
describe('Agent Configs', () => {
  describe('PR_REVIEW_AGENTS', () => {
    it('should have CODE_REVIEWER config', () => {
      ...
      expect(PR_REVIEW_AGENTS.CODE_REVIEWER.subagent_type).toBe('pr-review-toolkit:code-reviewer');
  ...
  describe('QUALITY_AGENTS', () => {
    it('should have SECURITY_AUDITOR config', () => {
      ...
      expect(QUALITY_AGENTS.SECURITY_AUDITOR.subagent_type).toBe('security-auditor');
    });

    it('should have ACCESSIBILITY_TESTER config', () => {
      expect(QUALITY_AGENTS.ACCESSIBILITY_TESTER).toBeDefined();
      expect(QUALITY_AGENTS.ACCESSIBILITY_TESTER.subagent_type).toBe('accessibility-tester');
    });

    it('should have PERFORMANCE_ENGINEER config', () => {
      expect(QUALITY_AGENTS.PERFORMANCE_ENGINEER).toBeDefined();
      expect(QUALITY_AGENTS.PERFORMANCE_ENGINEER.subagent_type).toBe('performance-engineer');
    });

    it('should have ARCHITECT_REVIEWER config', () => {
      expect(QUALITY_AGENTS.ARCHITECT_REVIEWER).toBeDefined();
      expect(QUALITY_AGENTS.ARCHITECT_REVIEWER.subagent_type).toBe('architect-reviewer');
    });

    it('should have REFACTORING_SPECIALIST config', () => {
      expect(QUALITY_AGENTS.REFACTORING_SPECIALIST).toBeDefined();
      expect(QUALITY_AGENTS.REFACTORING_SPECIALIST.subagent_type).toBe('refactoring-specialist');
    });
  });
  ...
  describe('ALL_AGENTS', () => {
    it('should include all PR review agents', () => {
      ...
      expect(ALL_AGENTS.SILENT_FAILURE_HUNTER).toBe(PR_REVIEW_AGENTS.SILENT_FAILURE_HUNTER);
    });
