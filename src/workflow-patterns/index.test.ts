...
describe('Workflow Patterns', () => {
  ...
  describe('generateBranchName', () => {
    it('should generate feature branch names', () => {
      expect(generateBranchName('feature', 'dashboard')).toBe(
        'feature/dashboard'
      )
    })
    ...
    it('should remove existing prefix', () => {
      expect(generateBranchName('feature', 'fix-something')).toBe(
        'feature/something'
      )
    })
  ...
  describe('determineVersionBump', () => {
    ...
    it('should return major for breaking changes', () => {
      expect(determineVersionBump(['feat: BREAKING CHANGE: new API'])).toBe(
        'major'
      )
    ...
    it('should prioritize features over patches', () => {
      expect(determineVersionBump(['fix: bug fix', 'feat: new feature'])).toBe(
        'minor'
      )
    })
  })
  ...
  describe('applyVersionBump', () => {
    ...
    it('should throw on invalid version', () => {
      expect(() => applyVersionBump('invalid', 'patch')).toThrow(
  ...
  describe('parseCommitToChangelogEntry', () => {
    ...
    it('should parse feat commits as Added', () => {
      const entry = parseCommitToChangelogEntry(
        'abc123',
        'feat: add dashboard',
        testDate
      )
