...
export interface UserServiceClient {
  ...
  updateUser(userId: string, updates: Partial<UserData>): Promise<UserData | null>;
  upgradeToTier(
    userId: string,
    tier: UserTier,
    stripeCustomerId: string,
  ...
  checkUsageLimit(userId: string, limits: Record<UserTier, number>): Promise<UsageCheck | null>;
...
class UserService implements UserServiceClient {
  ...
  async createUser(userId: string, email: string): Promise<UserData | null> {
    ...
    try {
      ...
      await this.client!.set(`${this.keyPrefix}email:${email.toLowerCase()}`, userId);
  ...
  async getUserByStripeCustomerId(stripeCustomerId: string): Promise<UserData | null> {
    ...
    try {
      const userId = await this.client!.get<string>(`${this.keyPrefix}stripe:${stripeCustomerId}`);
      if (!userId) return null;
      return this.getUser(userId);
    } catch (error) {
  ...
  async updateUser(userId: string, updates: Partial<UserData>): Promise<UserData | null> {
    ...
    try {
      ...
      if (updates.stripeCustomerId && updates.stripeCustomerId !== existing.stripeCustomerId) {
        await this.client!.set(`${this.keyPrefix}stripe:${updates.stripeCustomerId}`, userId);
      }
  ...
  async incrementUsage(userId: string, count: number = 1): Promise<number | null> {
    if (!this.isAvailable()) return null;
  ...
  async checkUsageLimit(
    ...
    const now = new Date();
    const trialExpired = user.tier === 'trial' && now > new Date(user.trialExpiresAt);
  ...
  async getOrCreateUser(userId: string, email: string): Promise<UserData | null> {
    const existing = await this.getUser(userId);
...
export function createUserService(config?: UserServiceConfig): UserServiceClient {
