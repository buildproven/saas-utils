...
describe('branded-types', () => {
  describe('createCommitHash', () => {
    ...
    it('should throw for empty string', () => {
      expect(() => createCommitHash('')).toThrow('CommitHash must be a non-empty string');
    });

    it('should throw for non-string input', () => {
      // @ts-expect-error - Testing invalid input
      expect(() => createCommitHash(123)).toThrow('CommitHash must be a non-empty string');
    });
    ...
    it('should throw for too long hash (> 40 chars)', () => {
      expect(() => createCommitHash('abc1234567890def1234567890abcdef123456789')).toThrow(
        'CommitHash must be a 7-40 character hexadecimal string',
      );
    });
  ...
  describe('createSemanticVersion', () => {
    ...
    it('should throw for empty string', () => {
      expect(() => createSemanticVersion('')).toThrow('SemanticVersion must be a non-empty string');
    });
    ...
    it('should throw for invalid format (missing patch)', () => {
      expect(() => createSemanticVersion('v1.2')).toThrow('SemanticVersion must match format');
    });

    it('should throw for invalid format (non-numeric)', () => {
      expect(() => createSemanticVersion('v1.x.3')).toThrow('SemanticVersion must match format');
    });

    it('should throw for invalid format (no dots)', () => {
      expect(() => createSemanticVersion('v123')).toThrow('SemanticVersion must match format');
    });

    it('should throw for invalid format (extra dots)', () => {
      expect(() => createSemanticVersion('v1.2.3.4')).toThrow('SemanticVersion must match format');
    });
  });
  ...
  describe('createPercentage', () => {
    ...
    it('should throw for non-number input', () => {
      // @ts-expect-error - Testing invalid input
      expect(() => createPercentage('50')).toThrow('Percentage must be a number');
    });

    it('should throw for NaN', () => {
      expect(() => createPercentage(NaN)).toThrow('Percentage must be a finite number');
    });

    it('should throw for Infinity', () => {
      expect(() => createPercentage(Infinity)).toThrow('Percentage must be a finite number');
    });
  ...
  describe('isCommitHash', () => {
    it('should return true for valid commit hash', () => {
      ...
      expect(isCommitHash('abc1234567890def1234567890abcdef12345678')).toBe(true);
    });
