...
export function getMaxApiCost(price: number, maxCostPercent: number = 10): number {
...
class UsageStore {
  ...
  getPurchases(userId: string, productType: 'validation' | 'starter-kit'): UserPurchase[] {
    const userPurchases = this.purchases.get(userId) || [];
  ...
  incrementUsage(userId: string, purchaseId: string, cost: number = 0): boolean {
    const userPurchases = this.purchases.get(userId);
...
export class ProductRateLimiter {
  ...
  async checkValidationLimit(
    ...
    const activePurchase = this.usageStore.getActivePurchase(userId, 'validation');
   ...
   */
  recordCost(userId: string, productType: 'validation' | 'starter-kit', cost: number): boolean {
  ...
  checkCostLimit(
    ...
    const activePurchase = this.usageStore.getActivePurchase(userId, productType);
  ...
  async checkDeepResearchLimit(userId: string): Promise<ProductLimitResult> {
    ...
    // Check monthly limit
    const monthlyUsage = this.usageStore.getMonthlyUsage(userId, 'deep-research');
  ...
  async checkStarterKitLimit(
    ...
    // Check if user has an active purchase with remaining runs AND cost budget
    const activePurchase = this.usageStore.getActivePurchase(userId, 'starter-kit');
  ...
  getValidationUsage(userId: string): ProductUsage {
    ...
    const totalRuns = PRODUCT_LIMITS.validation.runsPerPurchase * purchases.length;
    ...
    const activePurchase = purchases.find((p) => p.runsUsed < p.maxRuns && p.costUsed < p.maxCost);
  ...
  getStarterKitUsage(userId: string): ProductUsage {
    ...
    const totalRuns = PRODUCT_LIMITS.starterKit.scaffoldsPerPurchase * purchases.length;
    const usedRuns = purchases.reduce((sum, p) => sum + p.runsUsed, 0);
    const totalCost = purchases.reduce((sum, p) => sum + p.costUsed, 0);
    const totalMaxCost = purchases.reduce((sum, p) => sum + p.maxCost, 0);
    const activePurchase = purchases.find((p) => p.runsUsed < p.maxRuns && p.costUsed < p.maxCost);
