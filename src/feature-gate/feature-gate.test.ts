...
describe('Feature Gate', () => {
  const gate = createFeatureGate({
    tiers: {
      ...
      enterprise: {
        features: ['basic', 'manual', 'advanced', 'api', 'sso', 'audit'] as const,
...
describe('Edge Cases', () => {
  describe('Invalid Tier Handling', () => {
    ...
    it('should handle non-existent tier in getTierFeatures', () => {
      const { included, notIncluded } = gate.getTierFeatures('invalid' as 'basic');
  ...
  describe('Quota Edge Cases', () => {
    ...
    it('should handle Infinity limit correctly', () => {
      const result = gate.checkQuota('unlimited', 'requests', Number.MAX_SAFE_INTEGER);
      expect(result.allowed).toBe(true);
      expect(result.unlimited).toBe(true);
    });
  ...
  describe('Feature Inheritance Patterns', () => {
    const hierarchicalGate = createFeatureGate({
      tiers: {
        ...
        enterprise: {
          features: ['basic', 'export', 'api', 'analytics', 'sso', 'audit'] as const,
        },
      },
      upgradePath: {
        free: 'starter',
    ...
    it('should include all features for enterprise', () => {
      const { included, notIncluded } = hierarchicalGate.getTierFeatures('enterprise');
