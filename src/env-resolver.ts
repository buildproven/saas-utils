...
export function resolveOpItem(itemRef: string): string | null {
  try {
    ...
    const result = execFileSync('op', ['item', 'get', itemRef, '--reveal', '--format', 'json'], {
      encoding: 'utf8',
    ...
    const apiKeyField = item.fields?.find(
      (f: { label?: string }) =>
        f.label === 'credential' || f.label === 'api_key' || f.label === 'API Key',
    );
...
export function resolveApiKey(
  ...
  if (envValue && isOpPlaceholder(envValue)) {
    ...
    if (itemRef) {
      ...
      if (!silent) {
        console.log(`Could not resolve from 1Password. Run: op item get ${itemRef} --reveal`);
      }
    }
  }
  ...
  const envFiles = [
    ...(allowCwdEnv
      ? [path.join(process.cwd(), '.env'), path.join(process.cwd(), '.env.local')]
      : []),
